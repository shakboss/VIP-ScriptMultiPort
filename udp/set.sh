...
source <(curl -sSL 'https://raw.githubusercontent.com/shakboss/VIP-ScriptMultiPort/main/udp/module/module')
...
if [ "$(lsb_release -rs)" = "8*|9*|10*|11*|16.04*|18.04*" ]; then
 ...
 # [+get files ⇣⇣⇣+]
  source <(curl -sSL 'https://raw.githubusercontent.com/shakboss/VIP-ScriptMultiPort/main/udp/module/module') &>/dev/null
  wget -O /etc/UDPCustom/module 'https://raw.githubusercontent.com/shakboss/VIP-ScriptMultiPort/main/udp/module/module' &>/dev/null
  chmod +x /etc/UDPCustom/module

  wget "https://raw.githubusercontent.com/shakboss/VIP-ScriptMultiPort/main/udp/bin/udp-custom-linux-amd64" -O /root/udp/udp-custom &>/dev/null
  # wget "https://raw.githubusercontent.com/shakboss/VIP-ScriptMultiPort/main/udp/bin/udp-request-linux-amd64" -O /usr/bin/udp-request &>/dev/null
  chmod +x /root/udp/udp-custom
  # chmod +x /usr/bin/udp-request

  wget -O /etc/limiter.sh 'https://raw.githubusercontent.com/shakboss/VIP-ScriptMultiPort/main/udp/module/limiter.sh'
  cp /etc/limiter.sh /etc/UDPCustom
  chmod +x /etc/limiter.sh
  chmod +x /etc/UDPCustom
  
  # [+udpgw+]
  wget -O /etc/udpgw 'https://raw.githubusercontent.com/shakboss/VIP-ScriptMultiPort/main/udp/module/udpgw'
  mv /etc/udpgw /bin
  chmod +x /bin/udpgw

  # [+service+]
  wget -O /etc/udpgw.service 'https://raw.githubusercontent.com/shakboss/VIP-ScriptMultiPort/main/udp/config/udpgw.service'
  wget -O /etc/udp-custom.service 'https://raw.githubusercontent.com/shakboss/VIP-ScriptMultiPort/main/udp/config/udp-custom.service'
  ...
  # [+config+]
  wget "https://raw.githubusercontent.com/shakboss/VIP-ScriptMultiPort/main/udp/config/config.json" -O /root/udp/config.json &>/dev/null
  chmod +x /root/udp/config.json

  # [+menu+]
  wget -O /usr/bin/udp 'https://raw.githubusercontent.com/shakboss/VIP-ScriptMultiPort/main/udp/module/udp' 
  chmod +x /usr/bin/udp
